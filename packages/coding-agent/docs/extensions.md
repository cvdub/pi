...
- `acpPreformattedText: true` marks the tool's text output as preformatted rather than markdown, wrapping each emitted text block in a fenced code block. ACP leaves `content` rendering to the client, so a client that renders markdown otherwise reinterprets raw output — a `* ` line becomes a bullet, `_x_` becomes emphasis, a leading `#` becomes a heading. The fence is always longer than any backtick run inside the block, and it is applied after the display bound and charged against it, so framing can neither be severed by truncation nor push a fragment past the limit. Leave it unset for tools whose text output is genuinely markdown.

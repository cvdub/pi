...
| `fastMode` | boolean | `false` | Use priority service for `openai-codex/gpt-*` models. Configure interactively with `/fast on`, `/fast off`, or `/fast status`; `--fast` and `--no-fast` override it for one run. |
